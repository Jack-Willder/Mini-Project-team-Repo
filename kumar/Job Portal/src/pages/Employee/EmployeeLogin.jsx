...
function EmployeeLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/emp/login", {
        email: loginEmail,
        password: loginPassword,
      });

      localStorage.setItem("empToken", res.data.token);
      localStorage.setItem("empData", JSON.stringify(res.data.jobseeker));
      navigate("/empprofile");
    } catch (err) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/emp/register", {
        name: registerName,
        email: registerEmail,
